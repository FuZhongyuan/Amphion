#!/usr/bin/env python3
"""
Preprocess semantic features for LJSpeech dataset to avoid real-time extraction during training.
...
def main():
    parser = argparse.ArgumentParser(description="Preprocess semantic features for LJSpeech dataset")
    parser.add_argument("--config", type=str, required=True, help="Path to preprocessing config JSON")
    parser.add_argument("--dataset", type=str, default="ljspeech", choices=["ljspeech", "libritts"],
                       help="Dataset type")
    ...
    for wav_file in wav_files:
        ...
        relative_path = wav_file.relative_to(Path(data_root) / "wavs")
