...
"""
Semantic-to-Mel DiT Trainer for MaskGCT

Trains the Semantic-to-Mel DiT model that converts
discrete semantic tokens to continuous mel spectrograms using diffusion.
"""
...
class S2MelDiTTrainer(BaseTrainer):
    """Trainer for Semantic-to-Mel DiT model."""

    def __init__(self, args, cfg):
        super(S2MelDiTTrainer, self).__init__(args, cfg)
        self._build_input_models()

    def _build_model(self):
        """Build S2Mel DiT model."""
        model = SemanticToMelDiT(cfg=self.cfg.model.s2mel_dit)

        # Configure gradient checkpointing
        gradient_checkpointing = getattr(self.cfg.model.s2mel_dit, 'gradient_checkpointing', False)
    ...
    def _train_step(self, batch):
        """Training step for S2Mel DiT model."""
        train_losses = {}
        total_loss = 0
        train_stats = {}
    ...
    def _valid_step(self, batch):
        """Validation step for S2Mel DiT model."""
        valid_losses = {}
        total_loss = 0
        valid_stats = {}
