...
class LJSpeechDataset(torch.utils.data.Dataset):
    ...
    def load_cached_paths(self):
        ...
        # First, apply ljspeech_ratio if needed
        ...
        logger.info(f"LJSpeech {split_name} set loaded successfully, ratio: {self.ljspeech_ratio:.2f}")
    ...
    def load_path2meta(self):
        ...
        self.wav_id2meta = {}
        for path in tqdm(self.wav_paths, desc=f"Loading meta from {split_name} paths"):
            wav_id = os.path.basename(path).replace(".wav", "")
