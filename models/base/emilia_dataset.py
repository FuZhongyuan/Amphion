...
class EmiliaDataset(torch.utils.data.Dataset):
    ...
    def load_cached_paths(self):
        ...
        # First, apply emilia_ratio if needed
        if self.emilia_ratio < 1.0:
            ...
            all_json_paths = [all_json_paths[i] for i in selected_indices]
        ...
        self.json_paths = [all_json_paths[i] for i in selected_indices] if all_json_paths else []
        ...
        if self.cache_type == "path":
            ...
            for duration, phone_count in tqdm(
                ...
                desc=f"Calculating {split_name} frame indices",
                total=len(self.wav_path_index2duration)
            ):
                self.index2num_frames.append(duration * 50 + phone_count)

        logger.info(f"Emilia {split_name} set loaded successfully, ratio: {self.emilia_ratio:.2f}")
        logger.info(f"Number of {split_name} samples - wavs: {len(self.wav_paths)}, jsons: {len(self.json_paths)}")
