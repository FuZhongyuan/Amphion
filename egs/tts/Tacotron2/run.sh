...
if [ $running_stage -eq 3 ]; then
    if [ -z "$infer_expt_dir" ]; then
        echo "[Error] Please specify the experimental directory. The value is like [Your path to save logs and checkpoints]/[YourExptName]"
        exit 1
    fi
...
# # Train
# sh egs/tts/Tacotron2/run.sh --stage 2 --name tacotron2_ljspeech --gpu 4

# Inference:

# sh egs/tts/Tacotron2/run.sh --stage 3 \
#     --infer_expt_dir ckpts/tacotron2/tacotron2_ljspeech \
#     --infer_output_dir ckpts/tacotron2/tacotron2_ljspeech/result \
#     --infer_mode "single" \
#     --infer_text "This is a test of tacotron2 synthesis." \
